...
with `org-agenda-block-separator' characters)."
  ...
  (add-hook 'server-after-make-frame-hook #'fr/org-agenda-redo-after-frame-delayed))
...
(when (modulep! :lang markdown)
  ;; Add strict rule only for README.md
  (setq auto-mode-alist (assoc-delete-all "/README\\(?:\\.md\\)?\\'" auto-mode-alist))
...
(when (modulep! :lang sh +fish)
  (defun +fish-common-config (mode)
    (when (modulep! :lang sh +fish +lsp)
      ...
      (defun +fish-eglot-skip-semicolon (orig-fun &rest args)
        (unless (and (eq major-mode 'fish-mode) (= last-command-event 59))
          (apply orig-fun args)))
      (advice-add 'eglot--post-self-insert-hook :around #'+fish-eglot-skip-semicolon)))
...
(when (modulep! :email mu4e)
  (after! mu4e
    ...
    (setopt mu4e-main-hide-personal-addresses t)
...
(when (modulep! :app telega)
  ...
  (after! telega
    ...
    ;; Ignore messages from blocked senders.
