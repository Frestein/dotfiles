...
Includes:
- *EPG* - Low-level GnuPG library for cryptographic operations.
- *EPA* - High-level user interface for transparent file encryption and decryption.
- *plstore* - Encrypted credential storage backend using plist format.
- *auth-source* - Unified interface for retrieving credentials from various backends.
