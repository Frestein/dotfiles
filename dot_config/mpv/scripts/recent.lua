...
local xdg_state_home = os.getenv("XDG_STATE_HOME")
	or (
		is_windows and os.getenv("LOCALAPPDATA") -- Windows
		or (os.getenv("HOME") and utils.join_path(os.getenv("HOME"), ".local", "state") or "")
	)

local log_directory = utils.join_path(xdg_state_home, "mpv/recent")

if not mkdir(log_directory) then
	mp.msg.error("Failed to create log directory: " .. log_directory)
	return
end

o.log_path = utils.join_path(log_directory, o.log_path)
...
function get_ext(path)
	if is_protocol(path) then
		...
		return clean_path:match(".+%.(%w+)$") and clean_path:match(".+%.(%w+)$"):upper() or nil
	end
end
...
mp.add_key_binding(nil, "display-recent", display_list)
mp.add_key_binding(nil, "recent-save", function()
	write_log(false)
